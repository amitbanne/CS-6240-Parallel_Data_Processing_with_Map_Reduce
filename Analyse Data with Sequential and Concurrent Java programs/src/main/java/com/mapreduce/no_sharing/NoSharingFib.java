...
public class NoSharingFib {
	...
	// no sharing fibonacci starter method invoked by Loader class
	public static void noSharingFibStarter(List<String> unparsedData,int iterationCount){
	...
	private static void printData(List<Long> runningTimes) {
		...
		System.out.println("******* NO SHARING FIBONACCI RESULTS *******");
	...
	public static void mergeRecordsForThread(Map<String, Station> temperatureData) {
		...
		for(Entry<String,Station> stationEntry : temperatureData.entrySet()){
			...
			if(stationTemperatureMap.containsKey(stationEntry.getKey())){
				...
				Double existingTemperature = stationTemperatureMap.get(stationEntry.getKey()).getCumulativeTemperature();
				int existingRecordsCount = stationTemperatureMap.get(stationEntry.getKey()).getReadingCount();
				
				Double freshTemperature = stationEntry.getValue().getCumulativeTemperature();
				int freshRecordsCount = stationEntry.getValue().getReadingCount();
				
				// compute the updated TMAX and record count
				Double cumulativeTemperature = existingTemperature+freshTemperature;
				int cumulativeRecordsCount = existingRecordsCount+freshRecordsCount;
				
				Station station = stationTemperatureMap.get(stationEntry.getKey());
				...
				station.setCumulativeTemperature(stationEntry.getValue().getCumulativeTemperature());
				station.setReadingCount(stationEntry.getValue().getReadingCount());
				stationTemperatureMap.put(stationEntry.getKey(), station);
			}
		}
	}
