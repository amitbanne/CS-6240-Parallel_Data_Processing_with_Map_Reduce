...
public class NoLockFib {
	...
	// no lock fibonacci starter method invoked by Loader class
	public static void noLockFibStarter(List<String> unparsedData,int iterationCount){
	...
	private static void printData(List<Long> runningTimes) {
		...
		System.out.println("******* NO LOCK FIBONACCI RESULTS *******");
...
class WorkerThreadFib extends Thread {
	...
	public void run() {
		...
		for (String data : unparsedTemparatureData) {
			...
			if (NoLockFib.stationTemperatureMap.containsKey(stationId)) {
				...
				NoLockFib.stationTemperatureMap.put(stationId, stationData);
